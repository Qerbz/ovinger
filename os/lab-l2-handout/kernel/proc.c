...
/*void mlfq2_scheduler(void)
{
    struct proc *p;
    struct cpu *c = mycpu();

    if (!finished){
        printf("queuing pid: %d to queue 2\n",last_proc->pid);
        enqueue(q2,last_proc);
    }

    for (p = proc; p < &proc[NPROC]; p++)
    {
        if((p->state == RUNNABLE) && (!(mlfq_queue_exists(q1, p) || mlfq_queue_exists(q2, p)))){
            printf("queuing pid: %d\n",p->pid);
            enqueue(q1,p);
        }
    }
