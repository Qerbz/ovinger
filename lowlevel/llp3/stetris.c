...
//  writes error msg to stderr and returns err
int error_print(char* msg, int err){
  fprintf(stderr, "ERROR: %s\n", msg);
  return err;
}

// colors pixel of position x,y with color color
int color_pixel(int x, int y, int color) {
  // printf("%d,%d: %d\n",x,y,color);
  memset(screen + (x + y * 8) * 2, color, 2);
  return 0;
}
...
bool initializeSenseHat() {
  ...
  DIR* dev = opendir("/dev");
  if (dev == NULL)
    return error_print("couldn't find dev directory", 0);
  struct dirent* dev_entry;
  
  // Check all fbs if they are the right one
  while((dev_entry = readdir(dev)) != NULL){
    if (strncmp(dev_entry->d_name, "fb", 2) == 0) {
      char path[256];
      sprintf(path, "/dev/%s", dev_entry->d_name);
      fd = open(path, O_RDWR);
      if(fd < 0)
        continue;
      int ret = ioctl(fd, FBIOGET_FSCREENINFO, &fb_info);
      if (ret < 0)
        continue;
  ...
  if (strcmp(fb_info.id, "RPi-Sense FB") != 0)
    return error_print("could not find framebuffer", 0);

  screen = mmap((void *)fb_info.smem_start, fb_info.smem_len, PROT_READ | PROT_WRITE, MAP_SHARED, fd, (off_t)0);
  if(screen < 0)
    return error_print("could not mmap screen", 0);

  // Joystick
  DIR* inputdir = opendir("/dev/input");
  if (inputdir == NULL)
    return error_print("couldn't find dev/input directory", 0);
  struct dirent* input_entry;
  
  // Check all events if they are the right one
  char path[256];
  while((input_entry = readdir(inputdir)) != NULL){
    if (strncmp(input_entry->d_name, "event", 5) == 0) {
      sprintf(path, "/dev/input/%s", input_entry->d_name);
      fd = open(path, O_RDWR);
      if(fd < 0)
        continue;
      char name[256];
      int ret = ioctl(fd, EVIOCGNAME(256), &name);
      if (ret < 0)
        continue;
  ...
  if(pfd.fd < 0)
    return error_print("could not open event stream", 0);
...
void freeSenseHat() {
  // clear screen
  memset(screen, 0, fb_info.smem_len);

  // freeing some stuffs
  munmap((void *)fb_info.smem_start, fb_info.smem_len);
  close(pfd.fd);
  close(fd);
}
...
int readSenseHatJoystick() {
  ...
  if (ret < 0)
    return error_print("couldn't poll joystick", -1);

  if (ret == 0) 
    return 0;

  if(read(pfd.fd, &buf, sizeof(buf)) < 0)
    return error_print("read", -1);

  if (buf.type != EV_KEY || buf.value == 0 || (buf.code != KEY_ENTER && buf.code != KEY_UP && buf.code != KEY_DOWN && buf.code != KEY_LEFT && buf.code != KEY_RIGHT))
    return 0;

  return buf.code;
}

static inline int tileOccupied(coord const target);
...
void renderSenseHatMatrix(bool const playfieldChanged) {
  ...
  // Write every tile with corresponding color if update occured
  if (playfieldChanged){
    for (int x = 0; x<8; x++){
      for (int y = 0; y<8; y++){
        coord const checkTile = {x,y};
        color_pixel(x,y,tileOccupied(checkTile));
      }
    }
  }
}
...
static inline void newTile(coord const target) {
  ...
  game.playfield[target.y][target.x].color = game.color;
  game.color = (((game.color) % 0xff  + 0b101101 ) | 0xc0) & 0xff;
}
